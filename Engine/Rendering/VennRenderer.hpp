...
extern enum VennEnum
{
	...
	V_STENCIL_BUFFER_BUT = GL_STENCIL_BUFFER_BIT,
	V_STENCIL_TEST = GL_STENCIL_TEST,
	V_ALWAYS = GL_ALWAYS,
	V_REPLACE = GL_REPLACE,
	V_KEEP = GL_KEEP,
	V_EQUAL = GL_EQUAL,
	V_NOTEQUAL = GL_NOTEQUAL,
};
...
public:
	...
	void VennClearStencil( int s );
	void VennStencilFunc( VennEnum func, int ref, unsigned int mask );
	void VennStencilOp( VennEnum fail, VennEnum zfail, VennEnum zpass );
	void VennColorMask( unsigned char red, unsigned char green, unsigned char blue, unsigned char alpha );
...
//---------------------------------------------------------------------------
inline void VennRenderer::ApplyTransformation( const Matrix4& transform )
{
	m_matrixStack.back().ApplyTransformationMatrix( transform );
}
