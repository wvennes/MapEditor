...
STATIC unsigned int							Material::s_currentBoundProgramID = 0;
...
STATIC Material* Material::CreateAndGetMaterial( const std::string& name, ShaderProgram* shaderProgram, const std::vector< std::string >& textureFilePaths )
{
	...
	{
		return s_materialRegistry[ name ];
	}
	else
	{
		result = new Material( shaderProgram, textureFilePaths );
		s_materialRegistry.insert( std::make_pair( name, result ) );
	}
...
//---------------------------------------------------------------------------
STATIC unsigned int Material::GetBoundShaderProgramID()
{
	return s_currentBoundProgramID;
}
...
void Material::UseProgram()
{
	...
	s_currentBoundProgramID = m_shaderProgram->m_programID;
}
