...
TextRenderer::~TextRenderer()
{
	if ( m_bitmapFont && m_isBitmapFontStored ) delete m_bitmapFont;
}
...
STATIC TextRenderer* TextRenderer::CreateAndGetTextRenderer( const char* fontMetaDataFilePath )
{
	...
	result->m_isBitmapFontStored = false;
	result->m_bitmapFont = BitmapFont::CreateOrGetFont( fontMetaDataFilePath );
...
//---------------------------------------------------------------------------
float TextRenderer::CalculateTextWidth( const char* text, const float cellHeight ) const
{
	std::string textString( text );
	std::map< int, GlyphMetaData >& glyphMetaData = m_bitmapFont->m_glyphData;
	float result = 0.f;
